...
class MRDataset(InMemoryDataset):
    ...
    def data_read(self, h="lh", nbr_of_subs=1000, connectomes=1):
        ...
        for i in range(nbr_of_subs):
            s = data_path + "/test"
            s +=str(i) + "_"

            
             # Sometimes subject we are looking for does not exist
            t0 = np.loadtxt(s + "t0.txt")
            t1 = np.loadtxt(s +  "t1.txt")
            t2 = np.loadtxt(s + "t2.txt")
...
#t0_alone=np.dstack((t0[:read_limit, :]))
        #if subjects_t0 is None:  # If first subject
        #            subjects_t0 = t0_alone
        #else:
         #   subjects_t0 = np.vstack((subjects_t0, t0_alone))
        
        
        #t1_alone=np.dstack((t1[:read_limit, :]))
        #if subjects_t1 is None:  # If first subject
        #            subjects_t1 = t1_alone
        #else:
        #    subjects_t1 = np.vstack((subjects_t1, t1_alone))
        #t2_alone=np.dstack((t2[:read_limit, :]))
        #if subjects_t2 is None:  # If first subject
        #            subjects_t2 = t2_alone
        #else:
        #    subjects_t2 = np.vstack((subjects_t2, t1_alone))
        #print(subjects_t0.shape)
        #print(subjects_t1.shape)
        #print(subjects_t2.shape)